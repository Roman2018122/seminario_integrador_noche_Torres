...
function imprimirEstado(texto: string): void {
  console.log(texto);
...
function procesoTallerActivo(): never {
  while (true) {
...
function calcularCostoTotal(cantidad: number, precio: number) {
  return cantidad * precio; // tipo inferido: number
}

// Pero el retorno explícito actúa de contrato:
function calcularCostoPromedio(total: number, servicios: number): number {
  if (servicios === 0) lanzarError("No existen servicios registrados"); // never encaja en cualquier tipo
  return total / servicios;
}


/*Ejemplo aplicado — manejador de errores de API del taller*/
...
function procesarError(codigo: CodigoHTTP): never {
  ...
  throw new Error(
    `HTTP ${codigo}: ${mensajes[codigo] ?? "error desconocido"}`
  );
}
...
try {
  ...
  console.log(
    `asegurar(-500 >= 0): lanzó → ${(e as Error).message}`
  );
}