...
function tablaCostos(costoServicio: number): void {
  console.log(`\n--- Tabla de costos del servicio de $${costoServicio} ---`);

  for (let i = 1; i <= 10; i++) {
    const costoTotal: number = costoServicio * i;

    console.log(
      `${i.toString().padStart(2)} vehículo(s) x $${costoServicio.toFixed(2)} = $${costoTotal.toFixed(2)}`
    );
  }
}
